...
Architecture:
  Input (N, T, F)
    ...
    → MAGEBlock × num_mage_layers                  [MaGNet temporal encoder]
        Mamba/SelectiveSSM -> SparseMoE -> MHA -> Z_temp (N, T, D)
    │
...
except Exception:  # optional CUDA/Linux dependency
    _MambaSSM = None
...
class SelectiveSSMFallback(nn.Module):
    """Pure-PyTorch Mamba-style selective SSM fallback.

    The official `mamba-ssm` package is used when installed. This fallback
    keeps the same sequence-mixer role without requiring custom CUDA builds.
    """
    ...
    def __init__(
        ...
        self.A_log = nn.Parameter(torch.log(A_init).repeat(self.d_inner, 1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, C, dt_seed = torch.split(
            params, [self.d_state, self.d_state, self.d_inner], dim=-1
        )
        dt = F.softplus(self.dt_proj(dt_seed))

        A = -torch.exp(self.A_log.float())                  # (inner, state)
        state = x.new_zeros(N, self.d_inner, self.d_state)
        outputs = []
        for t in range(T):
            dt_t = dt[:, t, :].unsqueeze(-1)                # (N, inner, 1)
            decay = torch.exp(dt_t * A.unsqueeze(0))        # (N, inner, state)
            B_t = B[:, t, :].unsqueeze(1)                   # (N, 1, state)
            x_t = x_conv[:, t, :].unsqueeze(-1)             # (N, inner, 1)
            state = decay * state + dt_t * B_t * x_t
            C_t = C[:, t, :].unsqueeze(1)                   # (N, 1, state)
            y_t = (state * C_t).sum(dim=-1) + self.D * x_conv[:, t, :]
            outputs.append(y_t)
...
class MambaMixer(nn.Module):
    """Mamba sequence mixer with an official implementation when available."""
    ...
    def __init__(
        ...
        if _MambaSSM is not None:
            ...
            self.impl_name = "mamba-ssm"
        else:
            self.impl = SelectiveSSMFallback(
                d_model=embed_dim,
                d_state=d_state,
                d_conv=d_conv,
                expand=expand,
            )
            self.impl_name = "torch-selective-ssm"
...
class MAGEBlock(nn.Module):
    """Mamba-Attention-Gating-Experts block (MaGNet).

    Mamba/SelectiveSSM -> SparseMoE -> Multi-head self-attention.
    Downstream TCH, GAT, GPH, semantic fusion, and training interfaces remain
    unchanged from the BiGRU+MoE baseline.
    """
    ...
    def __init__(self, embed_dim: int, num_experts: int = 4,
        ...
        self.mamba = MambaMixer(embed_dim=D)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        z_mamba = self.mamba(x)                            # (N, T, D)
        z_mamba = self.norm_mamba(self.drop(z_mamba) + x)

        z_moe = self.moe(z_mamba)
        z_moe = self.norm_moe(self.drop(z_moe) + z_mamba)
...
class HybridStockModel(nn.Module):
    ...
    Combines:
    - MAGE temporal encoder (Mamba+MoE) for per-stock sequence modelling
