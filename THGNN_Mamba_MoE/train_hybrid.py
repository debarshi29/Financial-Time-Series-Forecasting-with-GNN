...
class SplitIndices:
    ...
    holdout_end_exclusive: int | None = None
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--test-start-date",  type=str, default="2025-01-01",
                   help="Validation split used for checkpoint selection and early stopping.")
    p.add_argument("--test-end-date",    type=str, default="2026-12-31")
    # Optional holdout split — evaluated ONCE at the very end, never used for early stopping.
    # Use this for the clean out-of-sample test set to report in the thesis.
    # Example: --holdout-start-date 2024-01-01 --holdout-end-date 2026-02-28
    p.add_argument("--holdout-start-date", type=str, default=None)
    p.add_argument("--holdout-end-date",   type=str, default=None)
    # Training
...
def compute_split_indices(files: list[str], args: argparse.Namespace) -> SplitIndices:
    ...
    if not (train_start < train_end_exclusive <= test_start < test_end_exclusive):
        raise ValueError(
            ...
            f"val [{test_start}, {test_end_exclusive})."
        )
    ...
    if max_date < pd.to_datetime(args.test_end_date):
        print(f"Warning: val end {args.test_end_date} beyond data; "
              f"using {max_date.date()}.")

    # Optional holdout split — must come after the val split
    holdout_start: int | None = None
    holdout_end_exclusive: int | None = None
    if args.holdout_start_date and args.holdout_end_date:
        holdout_start        = _index_for_date(file_dates, args.holdout_start_date)
        holdout_end_exclusive = _exclusive_end_index_for_date(file_dates, args.holdout_end_date)
        holdout_end_exclusive = min(holdout_end_exclusive, len(files))
        if holdout_start < test_end_exclusive:
            raise ValueError(
                f"Holdout split [{args.holdout_start_date}, {args.holdout_end_date}] "
                f"overlaps with the validation split [{args.test_start_date}, {args.test_end_date}]."
            )
        if holdout_end_exclusive <= holdout_start:
...
def main() -> None:
    ...
    logger.info(
        f"Val:     {files[split.test_start]} -> {files[split.test_end_exclusive - 1]} "
        f"({split.test_end_exclusive - split.test_start} samples)  [used for checkpoint selection]"
    )
    if split.holdout_start is not None:
        logger.info(
            f"Holdout: {files[split.holdout_start]} -> {files[split.holdout_end_exclusive - 1]} "
            f"({split.holdout_end_exclusive - split.holdout_start} samples)  [evaluated ONCE at end]"
        )
    ...
    holdout_ds = None
    if split.holdout_start is not None:
        holdout_ds = AllGraphDataSampler(
            base_dir=str(args.data_dir),
            mode="val",
            data_start=split.train_start,
            data_middle=split.holdout_start,
            data_end=split.holdout_end_exclusive,
        )
        if len(holdout_ds) == 0:
            logger.warning("Holdout dataset is empty — check --holdout-start/end-date.")
            holdout_ds = None
        else:
            logger.info(f"Holdout dataset: {len(holdout_ds)} samples")
    ...
    test_loader    = DataLoader(test_ds,  batch_size=1, pin_memory=pin,
                                collate_fn=lambda x: x)
    holdout_loader = (DataLoader(holdout_ds, batch_size=1, pin_memory=pin,
                                 collate_fn=lambda x: x)
    ...
    if args.resume:
        ...
        _orig_posix = getattr(_pl, "PosixPath", None)
        try:
            if _orig_posix is not None:
                _pl.PosixPath = _pl.PurePosixPath  # type: ignore[attr-defined]
            ckpt = torch.load(resume_path, map_location=device, weights_only=False)
        finally:
            if _orig_posix is not None:
                _pl.PosixPath = _orig_posix  # type: ignore[attr-defined]
    ...
    final_test    = run_epoch(test_loader,  model, device, None, args,
                              return_scale=args.return_scale)
    # Holdout evaluated exactly once on the best checkpoint — never used for selection.
    final_holdout = (run_epoch(holdout_loader, model, device, None, args,
                               return_scale=args.return_scale)
                     if holdout_loader is not None else None)
    if final_holdout is not None:
        logger.info(
            f"HOLDOUT (untouched) | "
            f"loss={final_holdout['loss']:.6f}  "
            f"rank_ic={final_holdout['rank_ic']:.4f}  "
            f"dir_acc={final_holdout['dir_acc']:.4f}"
        )
    ...
    summary = {
        "run_tag":         run_tag,
        "split_note":      ("val split used for checkpoint selection; "
                            "holdout is untouched out-of-sample"
                            if final_holdout is not None
                            else "no holdout split — test used for checkpoint selection"),
        "stop_reason":     stop_reason,
        "best_epoch":      best_epoch,
        "best_val_ic":     round(best_test_ic, 6),
        "best_val_mse":    round(best_test_mse, 8),
        "final_train_loss":round(final_train["loss"], 6),
        "final_val_loss":  round(final_test["loss"], 6),
        "final_val_rank_ic":  round(final_test["rank_ic"], 6),
        "final_val_dir_acc":  round(final_test["dir_acc"], 6),
        "final_holdout_loss":    round(final_holdout["loss"], 6)    if final_holdout else None,
        "final_holdout_rank_ic": round(final_holdout["rank_ic"], 6) if final_holdout else None,
        "final_holdout_dir_acc": round(final_holdout["dir_acc"], 6) if final_holdout else None,
    ...
    logger.info(f"Best val IC:      {best_test_ic:.4f}  (used for checkpoint selection)")
    logger.info(f"Best val MSE:     {best_test_mse:.6f}")
    logger.info(f"Checkpoint:       {best_path}")
    logger.info(
        f"Final val metrics  |  "
        f"train_loss={final_train['loss']:.6f}  "
        f"val_loss={final_test['loss']:.6f}  "
        f"val_rank_ic={final_test['rank_ic']:.4f}  "
