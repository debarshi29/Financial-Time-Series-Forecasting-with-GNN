...
Usage
-----
# Default: plots most recent *_mamba_moe_best.dat checkpoint over a date range
...
python plot_live_predictions.py --start-date 2026-03-01 --end-date 2026-03-21 \\
    --checkpoint ../THGNN/data/model_saved/2025-09-30_mamba_moe_best.dat
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Real-time prediction plots for the Hybrid THGNN x MaGNet Mamba+MoE model.",
    ...
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="Path to a specific .dat checkpoint. "
                             "Defaults to the most recently modified *_mamba_moe_best.dat.")
...
def _resolve_checkpoint(checkpoint_arg: str | None) -> Path:
    if checkpoint_arg:
        ...
        return path
    candidates = sorted(MODEL_DIR.glob("*_mamba_moe_best.dat"), key=lambda p: p.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(
            f"No *_mamba_moe_best.dat found in {MODEL_DIR}. "
