"""
backtest_hybrid.py — Long-short backtest for the Hybrid THGNN × MaGNet model.
...
-----
    ...
    # Specific checkpoint
    python backtest_hybrid.py --checkpoint ../THGNN/data/model_saved/2023-12-29_hybrid_best.dat
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Backtest the Hybrid THGNN×MaGNet model with financial metrics."
    )
    ...
    p.add_argument("--checkpoint",  type=str, default=None,
                   help="Path to a *_hybrid_best.dat file. "
...
def _resolve_checkpoint(arg: str | None) -> Path:
    if arg:
        ...
        return p
    candidates = sorted(MODEL_DIR.glob("*_hybrid_best.dat"),
                        key=lambda x: x.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(f"No *_hybrid_best.dat found in {MODEL_DIR}")
    return candidates[-1]
...
def _plot_equity_curve(
    ...
    fig.suptitle(
        f"Hybrid THGNN×MaGNet Backtest{title_suffix}\n"
...
def _plot_quintiles(
    ...
    ax.set_title("Quintile Return Analysis — Hybrid THGNN×MaGNet")
...
def _plot_ic_histogram(ic_arr: np.ndarray, ic_mean: float, out_path: Path) -> None:
    ...
    ax.set_title(f"IC Distribution — Hybrid THGNN×MaGNet  (n={len(valid)} days)")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
...
def _plot_cost_sensitivity(
    ...
    fig.suptitle("Transaction Cost Sensitivity — Hybrid THGNN×MaGNet", fontsize=12)
...
def main() -> None:
    ...
    report_lines = [
        sep,
        "HYBRID THGNN x MaGNet  BACKTEST REPORT",
        sep,
        "",
...
def _plot_monthly_heatmap(
    ...
    ax.set_title("Monthly L-S Returns (%) — Hybrid THGNN×MaGNet")
